...
def test_dq_basic_expectations_drops_null_values(spark):
    data = [
        Row(accident_id="TTI-1", icon_category=1, impact_jams=1, start_time="2026-08-01", description="A", start_longitude=15.0, start_latitude=50.0), # true
...
def test_dq_business_expectations_filters_geography(spark):
    data = [
        Row(start_longitude=19.0, start_latitude=52.0, icon_category=1),  # Valid 
        Row(start_longitude=10.0, start_latitude=52.0, icon_category=1),  # Invalid Longitude 
        Row(start_longitude=19.0, start_latitude=40.0, icon_category=1)   # Invalid Latitude
    ]
    df = spark.createDataFrame(data)
